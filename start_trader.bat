...
rem -- niet dubbel starten: draait mt5_trader.py al? --------------------
"%PS%" -NoProfile -Command "exit ([bool](Get-CimInstance Win32_Process -Filter \"Name='python.exe'\" | Where-Object { $_.CommandLine -like '*mt5_trader.py*' }))"
if errorlevel 1 (
    echo mt5_trader.py draait al - niets te doen.
    goto :eof
)

echo Bot wordt gestart...
cd /d "%BOT_DIR%"
"%PYTHON_PATH%" mt5_trader.py
goto :eof
